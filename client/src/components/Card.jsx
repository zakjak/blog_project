...
function Card({ post }) {
  ...
  return (
    <div className='w-full relative flex flex-row flex-wrap opacity-85 rounded-t-lg drop-shadow-xl bg-white dark:bg-[#10172B] cursor-pointer hover:opacity-100 transition-all duration-100'>
      <Link onClick={handleClick}  to={`/article/${post._id}`} className=''>
          ...
          <div className=" mt-1 p-4 flex flex-col gap-1 justify-between items-start">
              ...
              <span className='bg-black px-2 rounded-md text-white text-sm tracking-wider'>{post?.category.charAt(0).toUpperCase() + post.category.slice(1)}</span>
          </div>
      </Link>
      {
